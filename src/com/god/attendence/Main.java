...
public class Main extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		refreshCaptcha.setOnClickListener(new View.OnClickListener() {			
			...
			public void onClick(View v) {
				new getImg().execute();	
				Log.i(Main.class.toString(), "Refreshing Captcha...");
			}
		});
		...
		login.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				try
				{
					...
					connection = client.open(new URL("https://academics.ddn.upes.ac.in/upes/index.php"));
					...
					{
						connection = client.open(new URL("https://academics.ddn.upes.ac.in/upes/index.php?option=com_stuattendance&task='view'&Itemid=7631"));
						connection.setRequestProperty("Accept-Charset", charset);
						connection.setRequestProperty("User-Agent", getString(R.string.UserAgent)); 
	...
	private class getImg extends AsyncTask<Void, Void, Bitmap>{
		...
		{	
			// TODO Block page while loading
			
			Log.i(Main.class.getName(), "Loading captcha image...");
			Bitmap icon = null;
			CookieHandler.setDefault(cookieMan);
			try 
			{
				URL newurl  = new URL("https://academics.ddn.upes.ac.in/upes/modules/create_image.php?");
				HttpsURLConnection urlConnection = (HttpsURLConnection) client.open(newurl);
		...
		protected void onPostExecute(Bitmap icon) {
			...
			Log.i(Main.class.getName(), "Loaded captcha image.");
		}
	}
	...
	private class getHiddenData extends AsyncTask<Void, Void, Void>{
		...
		{
			HttpURLConnection connection = null;
			Log.i(Main.class.getName(),"Collecting hidden data...");
			
			try 
			{									
				connection = client.open(new URL("https://academics.ddn.upes.ac.in/upes/"));
				connection.setRequestProperty("Accept-Charset", charset);
				connection.setRequestProperty("User-Agent", getString(R.string.UserAgent)); 
				InputStream response = connection.getInputStream();
				...
				Log.i(Main.class.getName(),"Status Code:"+connection.getResponseCode());
		...
		protected void onPostExecute(Void v) {
			Log.i(Main.class.getName(), "Collected hidden data.");
		}
	}
